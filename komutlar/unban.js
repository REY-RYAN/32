...
exports.run = async(client, message, args) => {
  ...
  if(!args[0]) return message.reply('Kaldırılacak banlı kullanıcının IDsini girmelisin. (Tüm banları kaldırmak için all yazmalısın)')
    if(args[0] === "all") {
    ...
    } catch(err) { message.reply('Belirtilen ID numarasının banı kaldırılamadı!').then(x => x.delete(5000)) }  { message.channel.send('Bir hata algılandı !') }
};
